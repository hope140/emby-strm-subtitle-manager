...
本文件只保存经过官方接口、当前源码、测试、日志或真实运行证明，并且后续开发容易再次踩到的结论。完整过程和样本见 [Gate 0 实测报告](../records/acceptance/gate0-report.md)。
...
当缺少这种边缘样本会无限期阻断已有独立证据支撑的单源工作时，应通过 ADR 明确拆分门禁，不能在阶段报告里悄悄放宽：允许单源能力继续推进，同时让未经真实验收的多源路径 fail closed，并保留多源支持声明与启用的独立门禁。本项目的具体决策见 [ADR-005](../decisions/adr/005-conditional-d2-entry-without-live-multisource.md)。
