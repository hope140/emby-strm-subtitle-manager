...
本 ADR 记录路线和默认边界；当前 D1 的代码、自动化、C92 部署、公网 HTTPS 和 Movie/Episode STRM Canary 已有验收证据，C92 真实 Movie 版本组样本已找到，客户端 `AlternateMediaSources` 字段修正和本地回归测试已完成，但完整 API/UI/source 对应验收仍待完成，详见 [D1 部署验收报告](../d1-deployment-acceptance.md) 和 [D2 多版本 MediaSources 实测记录](../d2-multisource-c92-sample.md)。[ADR-005](005-conditional-d2-entry-without-live-multisource.md) 将“进入单源 D2”和“宣称真实多源搜索支持”拆为两个门禁：允许 D2 契约、实现和单源 Canary 继续推进，多源搜索在完整真实门禁前必须安全拒绝。D2 的详细契约和当前 D2-B1 后端状态见 [D2 搜索预览契约](../d2-search-preview-contract.md)；D3 及所有写能力的前置条件不变。
