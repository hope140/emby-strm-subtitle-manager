...
D3 代码现已完成专用样本 Add 的本地契约和自动化验证，包含会话 CSRF、`subtitle:write`、allowlist、Artifact 绑定、原子非覆盖版本文件、Emby Refresh/轮询、history/quarantine 和幂等操作 ID。真实 C92 Add、MediaStreams、字幕流和客户端读取仍需独立部署验收；真实验收通过前不调整默认 `write_enabled=false`，也不开放 Replace、Delete、Upload 或批量写入。完整请求/恢复契约见 [D3 专用样本 Add 契约](../d3-dedicated-add-contract.md)。
