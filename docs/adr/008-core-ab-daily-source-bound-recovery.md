...
2. 多 source Item 的 Search、Fetch、Preview、Upload、Add、Replace、Delete 与 Restore 都要求明确且精确的 `media_source_id`。History 的最小查询按 Item 读取，但每条返回记录均携带不可变的 Item/source 绑定，UI 仅显示当前已选 source 的记录。每一步重新读取 Item，重新选择 source；缺失、错误、重复或变化的 source 一律安全拒绝。
...
6. Upload 只通过现有 Validator 生成已绑定的 PreviewArtifact，忽略客户端文件名和 MIME 类型；Add/Replace 复用该 Artifact，不另建写入链。
