...
选择方案 C，作为 D2.5 排期项，目标是在 D3 写入能力和公开发布前完成。当前 D2 的共享 Bearer Token 继续作为过渡契约，直到 D2.5 实现和迁移验收完成。

### 管理员登录

- 每个部署实例配置自己的管理员用户名和密码，不提供通用默认密码。
- 密码从 Docker Secret 文件读取；不写入镜像、Git、日志、URL 或普通环境导出。具体哈希格式、文件名和初始化辅助命令在 D2.5 实现任务中固定。
...
- 需要新增会话存储、Cookie、CSRF、密码校验和登录限速测试。
...
1. D2.5-A：固定管理员凭据 Secret、初始化/轮换流程和配置错误码。
2. D2.5-B：实现登录会话、Cookie 属性、TTL、重启失效和失败限速；保留 Bearer 自动化兼容。
