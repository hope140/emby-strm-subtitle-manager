...
- 决策　[ADR-008](../../decisions/adr/008-core-ab-daily-source-bound-recovery.md)、[ADR-009](../../decisions/adr/009-strm-write-target-and-multisource-boundary.md)
...
- 实际运行、日志或可复现结果　本地 loopback Fake Emby 与真实 Playwright 浏览器 E2E 已运行。本评审完成后，候选提交另经授权在 C92 完成单源 STRM 的 Upload/Add/Replace/Delete/Restore、MediaStreams、官方字幕流、真实管理 UI Provider 链路和实际播放器读取，并恢复 closed；详见 [C92 单源 STRM 正式验收](../acceptance/core-ab-c92-acceptance-20260826.md) 与 [ASS/UI 修复 C92 验收](../acceptance/core-ab-ass-ui-fix-c92-acceptance-20260826.md)。该运行不包含普通本地媒体或多源 STRM 正向验收。
