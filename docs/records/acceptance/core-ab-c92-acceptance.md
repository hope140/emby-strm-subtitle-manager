...
根据 [ADR-008](../../decisions/adr/008-core-ab-daily-source-bound-recovery.md) 和当前 `ResolveWriteTarget` 契约，不能用可映射的 Item.Path 冒充选中 source 的路径。因而本轮没有执行 Search→Fetch→Preview→Add、Upload、Replace、Delete、Restore、Emby Refresh 或媒体目录写权限变更。
