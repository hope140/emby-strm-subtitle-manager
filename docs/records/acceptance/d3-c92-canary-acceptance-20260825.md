...
验收窗口关闭后，环境负责人又在手机端使用实际 Emby 客户端独立播放并确认新增字幕可读取。该结论来自用户在 2026-08-25 的明确实测反馈；本报告只记录“手机端读取通过”，不记录账号、设备标识、媒体名称、Item ID、截图或客户端私有数据。
...
- 后续顺序以 [当前状态与路线图](../../planning/current-status-and-roadmap.md) 为准；不能把本次单源 Add 直接扩展为全库写入。
