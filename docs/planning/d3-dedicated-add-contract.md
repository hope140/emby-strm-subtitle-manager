...
状态：本文件保留 D3.1 专用样本 Add 的实现与 C92 真实验收记录，Emby Web 与手机端实际客户端读取均已确认；验收记录见 [D3 C92 Canary 验收](../records/acceptance/d3-c92-canary-acceptance-20260825.md)。Core A/B 已在本地实现日常 gate、正向多 source、Replace、Upload、可恢复 Delete 和 Restore；当前实现契约见 [ADR-008](../decisions/adr/008-core-ab-daily-source-bound-recovery.md) 与 [Core A/B 实现评审](../records/reviews/core-ab-implementation-review.md)，不代表已部署或完成新的真实 C92 综合验收。
