...
范围：上游 commit `3335a9c95eec8e1664b7ab29368c34ce10f13575` 的静态代码取证，以及本次基线中可执行的局部测试。结论针对当前 SubBridge（SB，字幕桥）V1，V1 的事实约束仍以 [ADR-001](../decisions/adr/001-v1-uses-emby-remote-subtitle-bridge.md)、已接受的 [ADR-002](../decisions/adr/002-project-codebase-route.md)、[总体规划](master-plan.md) 和 Gate 0 为准。
...
因此，当前证据支持“新建轻量后端，选择性复用 Parser 核心和少量表现层经验”，不支持把 ChineseSubFinder 整仓作为 V1 主框架。具体决策和后果见 [ADR-002-project-codebase-route.md](../decisions/adr/002-project-codebase-route.md)。
