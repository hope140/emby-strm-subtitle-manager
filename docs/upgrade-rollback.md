...
升级或回滚本身不授权打开远程搜索或写入。若另行批准受控写入，应按对应的 Item/source、路径、Hash、Refresh、MediaStreams、字幕流和客户端读取门禁执行，并在结束时恢复 closed。现有真实证据的适用范围见[当前状态与后续路线图](current-status-and-roadmap.md)。
