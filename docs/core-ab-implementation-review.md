...
Core A 已完成管理员受控日常模式和显式 Item/source 绑定的 Search→Fetch→Preview→Add。多 source Item 必须选定 source；单 source 可省略或精确提交 source。Add 的目标 basename 来自所选 source 的安全映射路径，因而不依赖标题、默认 source 或 source 排序。
...
- D2 在 Search、Fetch、Preview 和 Upload 前重新读取 Item/source；多 source 缺失选择返回 `media_source_selection_required`，错误 source 返回 `media_source_mismatch`。
...
- Replace 先写入、Hash 回读、Refresh 并核验新字幕，再归档旧文件；后续核验或 history 失败时恢复旧文件并隔离新文件。
- Delete 复制、fsync、Hash 核对并非覆盖提交到私有 trash 后才移除媒体副本；Restore 重新读取 Item/source、检查 Hash 与同名冲突，再不覆盖恢复。history 只保存 `item`/`source` 的恢复目录类别，不保存媒体路径。
- Upload 仅接收 `file`、`media_source_id`、`language`，限制 multipart 体积，忽略原文件名与 MIME 类型；Validator 产出普通 PreviewArtifact，随后由 Add/Replace 消费。
- UI 只补 source 状态、Upload、Add、Replace、Delete、操作历史和 Restore。令牌/CSRF 仅留在页面内存，DOM、日志和响应不包含路径、原文件名、原始候选 ID、Token 或凭据。

## 自动化证据

- 单元测试覆盖日常 gate、Candidate/Artifact 的 Item/source 重绑定、多 source D2 正向流、source-specific Add、Replace 的归档/回滚、Delete/trash、Restore 冲突、操作 ID 与 Artifact Hash 冲突、以及恢复目录类别。
...
- 新增约束　可恢复操作必须保存已验证的目录类别而不是媒体路径；Restore 只能在重新解析当前 Item/source 后恢复至 `item` 或 `source` 的安全目录。
...
- 已搜索的文档和关键词　`AGENTS.md`、`architecture.md`、`lessons-learned.md`、`adr/`、`Core A`、`Core B`、`MediaSource`、`STRM`、`Restore`、`history`、`quarantine`、`Upload`。
- 是否更新已有结论　是；更新当前路线、架构入口、D2/D3 历史契约说明、文档索引和 ADR，并将可复用恢复边界写入维护经验。
