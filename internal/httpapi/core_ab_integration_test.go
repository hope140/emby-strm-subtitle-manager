...
// the fixture directory. It deliberately has two sources with distinct media
// basenames so the integration test cannot pass through source ordering.
...
func (f *coreABFakeEmby) GetItem(_ context.Context, itemID string) (domain.EmbyItem, error) {
	...
	return domain.EmbyItem{
		...
		Path:        "/emby/media/movie.strm",
		MediaSources: []domain.MediaSource{
			{ID: "source-a", Name: "Version A", Path: "/emby/media/version-A.mkv", IsDefault: &defaultSource, MediaStreams: &sourceAStreams},
			{ID: "source-b", Name: "Version B", Path: "/emby/media/version-B.mkv", MediaStreams: &sourceBStreams},
		},
	}, nil
}
...
func newCoreABHTTPServer(t *testing.T) (http.Handler, *coreABFakeEmby, string, *bytes.Buffer) {
	t.Helper()
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "movie.strm"), []byte("https://fixture.example/movie"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestCoreABHTTPFakeEmbyDailyMultiSourceFlow(t *testing.T) {
	...
	if added.Code != http.StatusOK || !strings.Contains(added.Body.String(), `"file_name":"version-A.subbridge.zh-CN.srt"`) {
		t.Fatalf("add = %d %s", added.Code, added.Body.String())
	}
	if _, err := os.Stat(filepath.Join(root, "version-A.subbridge.zh-CN.srt")); err != nil {
		t.Fatal(err)
	}
