...
D1.5 增加了同一 Go 服务内嵌的最小只读 Web UI。它只浏览既有媒体库、Movie/Episode 分页、媒体详情和字幕清单，不增加搜索或写能力；Token 只保存在当前页面内存。使用方式和公网 HTTP/HTTPS 边界见 [D1.5 最小只读 Web UI](docs/d1.5-readonly-ui.md)，部署前检查见 [D1.5 部署前预检](docs/d1.5-deployment-preflight.md)。

当前服务还提供 `/` 与 `/assets/{asset}` 的内嵌只读 UI、D1 的 7 个 GET API/运维路由，以及默认关闭并受 Canary allowlist 保护的三个 D2 POST 路由（Search、Fetch、Preview）。UI 只在服务端 health 明确报告远程搜索已启用且当前为单源 Movie/Episode 时展示 D2 控件；它不增加 Save、Refresh、下载或任何写操作。`/livez` 与只返回极小状态的 `/readyz` 公开；所有 `/v1/*` 要求独立 Bearer Token，Token 不接受 query 参数。`/readyz` 会实际探测 Emby，不能只根据进程存活返回就绪。应用凭据、`security.identity_key_file` 与 `security.api_auth_token_file` 三者分离，后两者不能复用 Emby API Key。D2 UI 的本地 Fake Emby 浏览器验收已通过，真实 Provider Canary 仍未执行，详见 [D2-B2 UI 评审](docs/d2-b2-readonly-ui-review.md)。
