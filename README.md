...
当前服务还提供 `/` 与 `/assets/{asset}` 的内嵌只读 UI、D1 的 7 个 GET API/运维路由，以及默认关闭并受 Canary allowlist 保护的三个 D2 POST 路由（Search、Fetch、Preview）。UI 只在服务端 health 明确报告远程搜索已启用且当前为单源 Movie/Episode 时展示 D2 控件；它不增加 Save、Refresh、下载或任何写操作。`/livez` 与只返回极小状态的 `/readyz` 公开；`/v1/auth/login` 使用 Compose environment 中的管理员凭据登录，其余 `/v1/*` 接受短期管理员会话或带只读 scope 的独立 Bearer，Bearer 不接受 query 参数。`/readyz` 会实际探测 Emby，不能只根据进程存活返回就绪。应用凭据、`security.identity_key_file`、`security.api_auth_token_file` 和管理员 environment 分离，管理员密码不能复用 Emby API Key。D2 UI 的本地 Fake Emby 浏览器验收已通过，真实 Provider Canary 仍未执行，详见 [D2-B2 UI 评审](docs/d2-b2-readonly-ui-review.md) 和 [D2.5 管理员认证](docs/d2.5-admin-auth.md)。
